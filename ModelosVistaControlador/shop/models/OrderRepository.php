...
class OrderRepository
{
    public static function getActiveOrderByUserID($userId)
    {
        try {
            ...
            $sql = "SELECT id FROM order WHERE user_id = $userId AND state = 0";
            $result = $bd->query($sql);
    ...
    public static function createOrder($userID)
    {
        try {
            ...
            $sql = "INSERT INTO order (user_id, state,datetime) VALUES ($userID, 0,'".$datetime."')";
    ...
    public static function updateOrderState($orderID,$newState)
    {
        try{
            ...
            $bd= Conectar::conexion();
            $sql = "UPDATE order SET state = $newState WHERE id = $orderID";
