...
class UserRepository
{
    ...
    public static function register($user, $password) {
        try {
            ...
            $existingUser = $bd->query('SELECT * FROM user WHERE username="' . $user . '"');
            ...
            $sql = 'INSERT INTO user (username, password, rol) VALUES ("' . $user . '", "' . $hashedPassword . '", 1)';
    ...
    public static function validar($u, $p) {
        try {
            $bd = Conectar::conexion();
            $result = $bd->query('SELECT * FROM user WHERE username="' . $u . '"');
    
            if ($datos = $result->fetch_assoc()) {
                if ($datos['password'] == md5($p)) {
                    return new UserModel($datos);
                }
            } else {
